class OrdersSQL:
    ...
    delete_data_query = " DELETE FROM `users_orders`" \
                        " WHERE phone_number = %s"
    check_trigger_status = " SELECT" \
                           " id, phone_number, client, number, date, status, amount, pay_link, pay_status, cooking_time_from, cooking_time_to, delivery_time_from, delivery_time_to, project, trade_point, trade_point_card, delivery_method, delivery_adress" \
                           " FROM `users_orders`" \
